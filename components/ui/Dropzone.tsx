...
import {UseSupabaseUploadReturn} from "@/hooks/useUploadToBucket";
...
const DropzoneContent = ({ className, action, isAvatar }: { className?: string, action?: () => void, isAvatar?: boolean }) => {
    const {
        files,
        setFiles,
        onUpload,
    ...
    if(isAvatar){
        const file = files[0]
        return (<div className={"flex items-center justify-center flex-col gap-5"}>
            {file?.preview ? <Image src={file.preview} width={100} height={100} alt={"avatar-preview"} className={"aspect-square rounded-full"} /> : <></>}
            {file && <Button onClick={onUpload} disabled={loading}>{loading ? "Uploading...":"Upload"}</Button>}
            </div>
        )
    }
            ...
            {files.map((file, idx) => {
                ...
                return (
                    ...
                    >
                        ...
                        {!loading && !isSuccessfullyUploaded && maxFiles > 1 && (
                            <Button
                                size="icon"
                                variant="link"
                ...
                <div className="mt-2">
                    ...
                    >
                        {loading ? (
                            ...
                            <>{maxFiles === 1 ? "Upload" :"Upload files"}</>
                        )}
                    </Button>
                </div>
            )}
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
    ...
    if (isSuccess || (maxFiles === 1 && files.length === 1)) {
        return null
    }
