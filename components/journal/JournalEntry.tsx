...
const JournalEntry = (
    ...
    return (
        <>
            ...
            {isAuthUser && !editMode && <div className={"absolute top-2 right-0"}>
                <Button variant={"ghost"} onClick={() => {setEditMode(true)}}><EditIcon className={"text-muted-foreground"}/></Button>
            </div>}
            <span className={`px-1 mr-2 rounded text-primary-foreground absolute top-3 ${userColor}`}>
                {userName}
            </span>
            <div className={"text-center text-xs w-full flex justify-center gap-2"}>{lastUpdated && <Button onClick={() => {setViewHistory(true)}} variant={"ghost"} className={"p-0 h-4"}><History className={"w-4 h-4 text-muted-foreground"} /></Button>} {entryDate}</div>
